...
| **C14** | *(a correction to this spec, not to plan2.)* §13 **M8**'s acceptance gates the milestone on a refusal this spec forbids — *"attempting a Level-2 document fails with a typed `ladder_level_2_required`"* | **Struck — superseded by §6.2**, rewritten by `fixes/001`: `ladder_level_2_required` is *"retained as a code and no longer raised"*. M8 is accepted on the retention instead — the class stays in the `WindowError` family carrying its code, nothing raises it, and a Level-2 document **plans** rather than being refused | the exclusion was measured over the real corpus rather than argued: 14 of 208 documents and 2,719 pages (48%) refused, the pilot `TC1E-SF` among them, so M2b failed at step 05 under it (§6.2). Between two sections of one spec the later and more specific one wins. The implementation already follows §6.2: `backend/vsir/ingest/window.py:116` keeps the code string, `backend/tests/unit/test_ladder_level_2.py:113` asserts the class stays in the `WindowError` family **and** that `raise LadderLevel2Required` appears nowhere under `backend/vsir/`, and `::test_a_document_with_no_contents_page_plans_at_ladder_level_2` plans a no-contents-page 150-page document at Level 2 |
| **C15** | *(a correction to this spec, not to plan2.)* §13 **M8**'s acceptance sizes the resume proof by absolute scale — *"a **generated large PDF** (≥ 700 windows …) survives a kill at window 700 and resumes without re-billing completed windows"* | **The figure is replaced by the operational criterion the property needs: a corpus large enough to force at least three window checkpoints**, so a kill leaves completed windows behind it and uncompleted ones in front and the resume can be shown to re-bill neither. What shipped is `data/fixtures/synthetic_large/` — 150 pages, 5 windows, `PAGE_COUNT = 150` pinned in `backend/vsir/eval/synthetic_large.py`. **This reduces the evidenced scale**: behaviour at corpus scale — lease renewal over hours, Qdrant at ≈21,000 points, cost — is **not evidenced** and stays with **OQ-5** (scale-out, §17), which this row does not close | 700 windows at the 30-page cap is ≈21,000 pages — a PDF nobody would commit, and a replay fixture (D10) would need 700 frozen S2 responses. The property itself is proved whole at five windows: `backend/tests/api/test_resume.py::test_resume_completes_without_rebilling` asserts the windows already `done` are exactly the ones the resumed run never calls the backend for, `::test_sigterm_checkpoint_loses_at_most_one_window` the checkpoint bound, and `::test_the_corpus_forces_at_least_three_window_checkpoints` the precondition; §6.3's durable cache (`backend/vsir/vlm/cached.py`) is what makes the completed windows free |
| **C16** | *(a correction to this spec, not to plan2.)* §2.4 ports `app/sparse.py` **as-is** — *"raw term frequencies + Qdrant `Modifier.IDF`"* — and §5.3 and D2 describe the two sparse surfaces the same way | **The document side stores BM25's tf component** — `tf·(k1+1) / (tf + k1·(1 − b + b·len(D)/avg_len))`, `k1 = 1.2`, `b = 0.75`, `avg_len` pinned per surface — and the **query side sends `1.0` per distinct term**. The idf factor is still never computed locally: `Modifier.IDF` stays mandatory and `core/indexed.py` still refuses a collection that does not declare it. `sparse.build()` is replaced by `build_document()` / `build_query()`, because a query built with document weights applies `k1` twice and a document built with query weights loses saturation, and **both mistakes present identically, as worse neighbours** | measured, not argued (`fixes/005`). Over the 56 pages of the one real ingest, the query *"Safeguard Detector individual sensors"* ranked the **only page that prints `individual sensors`** — 254 tokens — **9th**, behind prose pages repeating `sensors` 13, 9, 9 and 8 times; BM25 puts it **2nd**. `individual` already carried idf 2.79 against `sensors`' 0.693, so no re-weighting of the branch could recover it: the defect is inside the branch's own arithmetic. **D2's stated reason for raw counts is preserved exactly** — ingesting a document still changes no stored value, because `len(D)` is the document's own property and `avg_len` is a released pin, not a live corpus statistic. The correction keeps D2's reason and drops its letter. Every sparse assertion in the suite was about *shape*; not one was about *order*, which is why 1,473 green tests could not see it |
| **C17** | *(a correction to this spec, not to plan2.)* §6.6 names **four** fingerprint fields, `{embed_model, dim, distance, composition_version}`, and `ingest/fingerprint.py` warned that *"a fifth field is a release, not a config change"* | **Five — `sparse_version` is added**, and it did exactly what that warning said a fifth field would do: it re-keyed every collection and refused every existing one. That is the cost, and it is the correct cost for a change to how a vector is made | C16 makes the sparse recipe half client-side, and a BM25 collection and a raw-term-frequency collection are **byte-identical in shape** — same named vectors, same `Modifier.IDF`, every schema check in `core/indexed.py` passes. They differ only in what the stored floats *mean*, so a process configured for one and serving the other returns worse neighbours and no error: precisely the silent failure §6.6 exists to prevent. Without the field this fix would itself have become a §6.6-class defect |
...
| `app/sparse.py` | 42 | **port with changes (C16)** | the **tf half of BM25** per document (saturation + length normalisation) and `1.0` per distinct query term; the idf factor stays Qdrant's `Modifier.IDF`, which remains mandatory. Keep `dedupe()`. Raw term counts were the ported behaviour and are a ranking defect — `fixes/005` (D2, C16) |
...
| Source documents | **kept**, on the `VSIR_DOC_STORE` volume, one file per document as `<doc_id>@<revision>.pdf` (U029). Derivable from any `page_id`, so no payload carries a path (§5.3, A5); integrity is the run record's `content_hash`, and bytes that disagree with it are refused rather than served. **This is not a reversal of the row above** — that one is about *rasters*, which are still re-rendered on demand and never written down. Without it there was nothing to re-render them *from* | a miss is a typed `document_not_stored` naming the document, never a placeholder image and never a 500 (§11.3) |
...
VECTORS ───────────────── one point, three surfaces, fused by RRF at query time (D2)
  ...
  lexical    sparse, BM25 tf weights over `text`, Qdrant Modifier.IDF (C16)
...
The collection stores `{embed_model, dim, distance, composition_version, sparse_version}` (**five** fields — the
fifth added by C17). On mismatch: **refuse to upsert**, and refuse to serve: `ingest/fingerprint.require`
guards the write and `doctor.check_collection_fingerprint` fails `/ready`, because a process that only
**reads** embeds its queries under one recipe against vectors built under another and the sole symptom is
worse neighbours (§4.3). A model change is a new collection + full re-embed + alias swap, never an
in-place mix. A `sparse_version` change needs no re-embed — both sparse surfaces are derivable from
payload already stored — but there is no supported command that rewrites them in place (plan §4c P10).
...
|---|---|---|
| `POST /documents` | **ingestion over HTTP** — multipart PDF in, `202` and a `run_id` out. Not a second implementation of §6.1: it spools the bytes and runs `vsir ingest --run-id <id>`, the same subcommand from the same image, so §15 Factor XII holds and register **E1** cannot recur. Refuses `403 spend_not_permitted` while `VSIR_ALLOW_PAID` is unset and the release names a live model — the one place that variable decides anything. **A run started this way is resumable across an instance restart** from U029 onward: step 02 deposits the source document in the store (`<doc_id>@<revision>.pdf` on the `VSIR_DOC_STORE` volume), so `vsir ingest --resume <run_id>` resolves the bytes from the run record's `doc_id@revision` and `content_hash` and needs no path at all. Until U029 it was not: the spool went with the instance and nothing else named the file | bearer |
...
- **Acceptance:** F9 closed **and F12's revision half** — publishing 1.4 retires 1.3's points to
  ...
  and publishes
  nothing partial; **a generated large PDF** large enough to force **at least three window
  checkpoints** (C15 — not the 1,440-page manual, which a resume test wants deterministic rather
  than large, §6.2) survives a kill that leaves both completed and uncompleted windows and
  resumes without re-billing the completed ones; `ladder_level_2_required` is **retained in the
  `WindowError` family and never raised**, and a Level-2 document **plans** rather than being
  refused (C14, §6.2); the evaluation report states code exactness and abstention
  correctness per set.

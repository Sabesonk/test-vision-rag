...
| ~~**P10**~~ | **CLOSED by `fixes/008` (2026-09-11) — `vsir migrate sparse`.** Both sparse surfaces are rebuilt from stored payload: no model call, no re-embed, the dense vector never fetched, and the fingerprint restamped **last** so a kill leaves the collection refused rather than published half-rebuilt. It refuses any fingerprint difference outside `sparse_version` — those fields describe the dense vector, and stamping this release over them is the in-place mix §6.6 forbids wearing the costume of a repair. *Originally:* **A `sparse_version` change has no supported migration.** `fixes/005` made the sparse recipe client-side and versioned (§6.6, C17), so every collection built before it is refused at boot — correctly. The cheap repair needs no model call: both sparse surfaces are derived entirely from payload already stored, so they can be rebuilt in place (scroll, `update_vectors`, rewrite the fingerprint point). There is no `vsir` subcommand that does it, and §15 Factor XII means an operational action without one is not supported — so the only path is delete-and-re-ingest, which deletes the embedding cache with the collection and makes the re-embed a **live** call. | ~~unassigned~~ closed | Two documents and 58 pages is pennies. At the 208-document corpus scale it is a bill and an outage, for a change that touches no embedding. Found 2026-09-11 while applying `fixes/005`, closed the same day. |
| **P11** | **The only paid run's receipts are untracked.** `data/fixtures/live/` — 188 KB, the frozen S1/S2 receipts and page text of the 2026-09-10 live ingest — is neither committed nor gitignored. It is the sole evidence behind `fixes/005`'s two `avg_len` figures and the discovery of `fixes/006`, and one `git clean -xfd` removes it. | U013 | §12.1's premise is *"one paid ingest buys a permanent test corpus"*, and this one has bought a temporary one. `data/fixtures/legacy/` is the precedent for committing third-party document text; the decision is the project owner's, not this build's. |
| **P12** | **One replay fixture is keyed on a page order that retrieval chooses.** `synthetic_pdf.py`'s `ask_vision` entry freezes Loop 5's escalation of the two image-only pages, and the loop takes their order from the `skim_pages` ranking. §6.3 makes page order an input to `read_key` — deliberately, since a model shown the same sheets in a different order is being asked a different question — so a ranking change re-keys the entry. `fixes/005` did exactly that: `(2, 1)` became `(1, 2)` and five tests across three `ask` suites failed with `fixture_miss`. | unassigned | D10 worked (a miss is typed, never a fabricated response) and the repair is one regenerated fixture, so this is a maintenance cost rather than a defect. Two things make it worth recording: the failure surfaces four suites away from the change that caused it, and the generator **does not prune** — the superseded fixture stayed on disk, unreferenced, and had to be deleted by hand. If Loop 5 ordered its escalation set by page rather than by rank, the call would be stable under any future ranking change; that is a behaviour change to the answer loop and is the owner's call, not this fix's. Found 2026-09-11 while running the L2 suite for `fixes/008`. |

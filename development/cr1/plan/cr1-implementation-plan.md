...
| **P10** | **A `sparse_version` change has no supported migration.** `fixes/005` made the sparse recipe client-side and versioned (§6.6, C17), so every collection built before it is refused at boot — correctly. The cheap repair needs no model call: both sparse surfaces are derived entirely from payload already stored, so they can be rebuilt in place (scroll, `update_vectors`, rewrite the fingerprint point). There is no `vsir` subcommand that does it, and §15 Factor XII means an operational action without one is not supported — so the only path is delete-and-re-ingest, which deletes the embedding cache with the collection and makes the re-embed a **live** call. | unassigned | Two documents and 58 pages is pennies. At the 208-document corpus scale it is a bill and an outage, for a change that touches no embedding. Found 2026-09-11 while applying `fixes/005`. |
